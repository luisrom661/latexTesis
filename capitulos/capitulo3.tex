...
\begin{itemize}
   ...
   \item \textbf{Social Sprint Review:} se reserva un espacio de aproximadamente 10 minutos para presentar el Health Score y discutir su evolución durante el Sprint.
...
\begin{longtable}[H]{
   ...
   Se amplía con elementos de Deuda Social o habilitadores de salud del equipo: iniciativas destinadas a mejorar la comunicación, reducir \textit{smells} persistentes o fortalecer las habilidades sociales del grupo (por ejemplo: ``Establecer protocolos de \textit{code review} no tóxicos para mitigar \textit{Priggish Members}'').
   ...
   Ejemplo: ``Implementar sesiones semanales de \textit{lunch \& learn} sobre el subsistema X.
   Owner (Propietario): Developer A + voluntario rotativo.
   DoD (Definición de Hecho): Al menos tres desarrolladores pueden explicar la arquitectura básica.
   ...
   \textbf{Increment}       &
   Además de cumplir con la \textit{Definition of Done} técnica, cada Increment se evalúa también por su impacto en la Salud Social.
   ...
   Se analizan indicadores como variaciones en el Health Score, aparición o reducción de \textit{smells}, distribución del conocimiento (SNA) o niveles de carga emocional (horas extra, rotación).
...
\begin{enumerate}
   ...
   \item \textbf{Dashboard de Métricas Sociales:}
         Una visualización en tiempo real de los principales indicadores de la salud del equipo: evolución del Health Score, distribución de conocimiento (SNA), señales tempranas de burnout (por ejemplo, exceso de horas extra o variaciones en la velocidad individual), índice de seguridad psicológica (a partir de encuestas cada 2 o 3 Sprints) y equilibrio en la participación en la toma de decisiones.
...
\begin{itemize}
   ...
   \item Las estrategias para mejorar la confianza pueden diseñarse de forma más intencionada; por ejemplo, implementar sesiones de \textit{pair programming} para reforzar la percepción de habilidad, o promover retrospectivas más abiertas y sinceras para fortalecer la benevolencia.
...
\textbf{Capa 1 (Fundamentos)}: Estos tres fundamentos convergen para habilitar los principios empíricos de Scrum —Transparencia, Inspección y Adaptación— que, aplicados al ámbito social, dan origen al enfoque distintivo de SocialScrum.
...
Los \gls{feedback-loops} (líneas discontinuas) evidencian que el sistema es reflexivo: los resultados positivos retroalimentan los valores y principios que los generaron, haciendo que el ciclo empírico se auto-perfeccione de forma continua.
...
En la Figura~\ref{fig:arquitectura_integrada_final}, se presenta la arquitectura integrada completa del modelo SocialScrum, mostrando cómo las cuatro capas interactúan para formar un sistema cohesivo y funcional.
