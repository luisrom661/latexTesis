...
Esta diferencia es crucial: identificar \textit{community smells} implica un acto de vulnerabilidad voluntaria, no una respuesta condicionada por incentivos formales.

Pensémoslo así:

\begin{itemize}
    \item Los \textit{community smells} no se identifican a través de \acrfull{kpi} tradicionales ni mediante sistemas de recompensas.
...
\textit{Community smells} como \gls{cs-solution-defiance} o \gls{cs-organizational-rigidity} son síntomas claros de entornos donde la autonomía ha sido limitada o erosionada con el tiempo.

En SocialScrum, la autonomía se protege a través de la autoorganización genuina: los equipos deciden qué \textit{community smells} priorizar, cómo diseñar sus estrategias de mitigación y qué compromisos sociales incorporar en el \textit{Social Sprint Backlog}.
...
SocialScrum aborda esta necesidad a través de diversas estrategias: fomenta el \textit{pair programming} y las sesiones de conocimiento compartido, para que los miembros fortalezcan sus habilidades en un entorno de apoyo y aprendizaje mutuo; incorpora en las retrospectivas espacios para reconocer y celebrar los avances sociales, no solo los técnicos; y promueve la transparencia sobre las competencias individuales y colectivas, evitando la cultura del ``fingir saber''.
...
\textit{Community smells} como \gls{cs-social-isolation}, \gls{cs-lone-wolf} o \gls{cs-toxic-relationships} son expresiones directas de esta carencia de relacionalidad.
...
\begin{itemize}
    \item Los \textit{community smells} impactan tanto la confianza técnica —``¿puedo contar con que esta persona hará bien su trabajo?''— como la confianza social —``¿le importa genuinamente el bienestar del equipo?''—.
...
SocialScrum promueve el fortalecimiento de esta dimensión a través de prácticas como el \textit{pair programming}, las sesiones de intercambio de conocimiento y la documentación accesible y compartida.
...
Por el contrario, \textit{community smells} como \gls{cs-toxic-relationships} o \gls{cs-priggish-members} evidencian la erosión de esta dimensión, pues revelan entornos donde predomina la competencia egoísta, la desconfianza o la falta de empatía.
...
Solo cuando existe confianza genuina —basada en integridad y benevolencia— pueden emerger conversaciones honestas sobre problemas interpersonales, lo que a su vez permite detectar y abordar \textit{community smells} de raíz.
...
De ahí que \textit{community smells} como \gls{cs-radio-silence} o \gls{cs-org-silo} resulten tan nocivos: interrumpen los flujos comunicativos y debilitan los lazos interpersonales que sostienen la colaboración y la confianza en equipos distribuidos.
...
\begin{itemize}
    ...
    \item \textit{Community smells} generales, sin mencionar nombres ni casos específicos (``El equipo muestra señales de aislamiento social'').
...
\begin{itemize}
    \item Qué tipo de información se recopilará (Health Score, \textit{community smells}, reflexiones surgidas en las retrospectivas).
...
Cuando el Social Guide comparte métricas o \textit{community smells} a nivel organizacional, debe anonimizar toda la información para que ninguna persona pueda ser identificada. Por ejemplo:

\begin{itemize}
    \item \textbf{Prohibido:} ``El desarrollador A muestra signos de \textit{burnout} (agotamiento emocional 9/10)''.
    \item \textbf{Permitido:} ``El equipo presenta un \textit{community smell} activo: \textit{Work Exhaustion}. El Health Score promedio es de 4.2/10, y el valor ``Foco'' se encuentra en estado crítico (3/10)''.
\end{itemize}
...
El equipo tiene pleno derecho a revisar cualquier reporte o métrica que el Social Guide planee compartir fuera del grupo. En otras palabras, antes de comunicar el Health Score o un \textit{community smell} a nivel organizacional, el Social Guide debe mostrar al equipo exactamente qué información se va a divulgar y en qué formato.
