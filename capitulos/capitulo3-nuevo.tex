...
\begin{figure}[!htbp]
    ...
    \includegraphics[width=\textwidth]{recursos/socialscrum_vista_general_cycle.png}
...
En la Figura \ref{fig:socialscrum_modelo_conceptual} se muestra el modelo conceptual de SocialScrum.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{recursos/socialscrum_modelo_conceptual.png}
...
En SocialScrum, el \textit{Product Owner} mantiene su rol esencial de maximizar el valor del producto \cite{schwaber2020scrum}. Sin embargo, ahora cuenta con una nueva fuente de información proveniente del \textit{Social Guide}, quien le ofrece una visión clara sobre cómo la deuda social puede afectar la predictibilidad, la colaboración y la velocidad del equipo.

Gracias a esta perspectiva, el \textit{Product Owner} puede tomar decisiones de priorización más equilibradas y sostenibles, valorando no solo el impacto del producto en el negocio, sino también la salud y estabilidad del equipo que lo hace posible.
...
El \textit{Scrum Master} conserva su papel como facilitador del proceso y protector del equipo ante interrupciones externas \cite{schwaber2020scrum}. En el contexto de SocialScrum, colabora estrechamente con el \textit{Social Guide} para identificar de manera temprana los impedimentos sociales que puedan afectar el desarrollo del Sprint.

Mientras el \textit{Social Guide} aporta una mirada analítica sustentada en datos —por ejemplo, a través de análisis de redes sociales (SNA) o métricas socio-técnicas—, el \textit{Scrum Master} complementa esta visión con una comprensión más vivencial del grupo, promoviendo conversaciones difíciles y facilitando la resolución constructiva de conflictos \cite{saeeda2024nontechnicaldebt}.
...
Los \textit{Developers} mantienen su responsabilidad principal sobre la implementación técnica y la autogestión del trabajo durante el Sprint \cite{schwaber2020scrum}. No obstante, en SocialScrum asumen también un rol más activo en el cuidado de la salud social del equipo. Esto implica participar en la detección de \textit{community smells} durante las ceremonias, proponer acciones de mejora que se integren al \textit{Sprint Backlog Social} y colaborar con el \textit{Social Guide} aportando retroalimentación sobre aspectos como la comunicación, la colaboración o la distribución del conocimiento \cite{saeeda2024nontechnicaldebt}.

De esta manera, los \textit{Developers} se convierten en agentes directos del cambio social dentro del equipo, garantizando que las acciones e intervenciones propuestas sean realistas, aplicables y sostenibles en el contexto de su trabajo cotidiano.
...
\emph{Referencia:} el Principio de No-Sobrecarga se explica en detalle en el Capítulo 4, Sección 4.3, donde se presentan tablas de distribución del tiempo por evento y estrategias prácticas para optimizar aún más el uso del tiempo sin generar sobrecarga.
...
\begin{adjustwidth}{\parindent}{0pt}
    \begin{table}[!htbp]
        ...
        \renewcommand{\arraystretch}{1.2}
        \small
        \begin{tabularx}{\dimexpr\textwidth-\parindent\relax}
            {@{}p{3.5cm} c c X@{}}
            \toprule
            ...
            Sprint Review
                            & 2 h
                            & +10 min
                            & Social Guide (evaluación del \textit{Health Score})                                                   \\
...
\emph{Nota:} El principio de \textbf{No Sobrecarga} establece que las adaptaciones de SocialScrum deben integrarse dentro de los eventos existentes de Scrum, procurando no incrementar de forma significativa la duración total de las ceremonias. Este principio se explica en profundidad en el Capítulo 4, Sección 4.3.
...
La descripción detallada del proceso —que incluye facilitadores, tiempos sugeridos, preguntas guía y herramientas de apoyo— se expone en el Capítulo 4, Sección 4.4, donde se presenta el manual práctico para la implementación del modelo.
...
En SocialScrum, la autonomía se protege a través de la autoorganización genuina: los equipos deciden qué \textit{community smells} priorizar, cómo diseñar sus estrategias de mitigación y qué compromisos sociales incorporar en el \textit{Sprint Backlog Social}.
El \textit{Social Guide} actúa como facilitador del proceso, pero no como figura de autoridad. De esta manera, el equipo se mantiene como el verdadero responsable —y dueño— de su salud social.
...
SocialScrum busca revertirla fortaleciendo los espacios de vínculo humano —por ejemplo, a través de la Sprint Retrospective Social, donde el equipo reflexiona sobre cómo se siente trabajando junto, no solo sobre lo que entrega.
...
Por el contrario, community smells como \gls{cs-toxic-relationships} o \gls{cs-priggish-members} evidencian la erosión de esta dimensión, pues revelan entornos donde predomina la competencia egoísta, la desconfianza o la falta de empatía.
...
\begin{itemize}
    ...
    \item el cumplimiento verificable de los compromisos incluidos en el Sprint Backlog Social, y
...
Solo cuando existe confianza genuina —basada en integridad y benevolencia— pueden emerger conversaciones honestas sobre problemas interpersonales, lo que a su vez permite detectar y abordar community smells de raíz.
...
De ahí que community smells como \gls{cs-radio-silence} o \gls{cs-org-silo} resulten tan nocivos: interrumpen los flujos comunicativos y debilitan los lazos interpersonales que sostienen la colaboración y la confianza en equipos distribuidos.
...
La aplicación concreta de estos valores —a través de indicadores tanto cuantitativos como cualitativos, como escalas de medición, preguntas de evaluación y umbrales de acción— se explica con mayor detalle en el Capítulo 4, Sección 4.5.
...
\begin{adjustwidth}{\parindent}{0pt}
    \begin{table}[!htbp]
        ...
        \renewcommand{\arraystretch}{1.2}
        \small
        \begin{tabularx}{\dimexpr\textwidth-\parindent\relax}{@{}p{3.0cm} X X@{}}
            \toprule
...
La forma de aplicar esta medición —que incluye el conjunto completo de preguntas, las escalas utilizadas y el seguimiento de las tendencias a lo largo del tiempo— se explica con mayor detalle en el Capítulo 4, Sección 4.5: \textit{``Métricas de Salud Social''}.
...
Toda la información compartida durante los eventos sociales —como el Daily Social, la Sprint Retrospective Social o las sesiones de mediación— debe manejarse con total confidencialidad. Ningún dato puede usarse para evaluaciones de desempeño, decisiones de contratación o despido, ni para cualquier acción que afecte la situación laboral de una persona.
...
\begin{itemize}
    ...
    \item Community smells generales, sin mencionar nombres ni casos específicos (``El equipo muestra señales de aislamiento social'').
...
\begin{itemize}
    \item Qué tipo de información se recopilará (Health Score, community smells, reflexiones surgidas en las retrospectivas).
...
\begin{itemize}
    \item \textbf{Prohibido:} ``El desarrollador A muestra signos de burnout (agotamiento emocional 9/10)''.
    \item \textbf{Permitido:} ``El equipo presenta un community smell activo: Work Exhaustion. El Health Score promedio es de 4.2/10, y el valor ‘Foco’ se encuentra en estado crítico (3/10)''.
\end{itemize}
...
La implementación detallada de estas salvaguardas —incluyendo plantillas de acuerdos, protocolos de reporte y ejemplos prácticos— se presenta en el Capítulo 4, Sección 4.7: ``Consideraciones Éticas y Salvaguardas Obligatorias''.
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        ...
        component/.style={rectangle, draw=blue!75, fill=gray!10, text=black, text width=3.0cm, minimum height=4.2cm,
        ...
        \node[component] (tad) {%
            \footnotesize\textbf{TAD}\\[3pt]\rule{\linewidth}{0.4pt}\\[4pt]
            Autonomía\\[2pt]
            Competencia\\[2pt]
            Relacionalidad
        };
        ...
        \node[component, right=0.5cm of tad] (conf) {%
            \footnotesize\textbf{Confianza}\\[3pt]\rule{\linewidth}{0.4pt}\\[4pt]
            Habilidad\\[2pt]
            Benevolencia\\[2pt]
            Integridad
        };
        ...
        \node[component, right=0.5cm of conf] (vals) {%
            \footnotesize\textbf{Valores de Scrum}\\[3pt]\rule{\linewidth}{0.4pt}\\[4pt]
            Coraje\\[2pt]
            Apertura\\[2pt]
            Respeto\\[2pt]
            Compromiso\\[2pt]
        ...
        \node[rectangle, draw=blue!75, fill=gray!10, text=black, minimum height=1.5cm, rounded corners,
            align=center, font=\footnotesize\bfseries, above=0.5cm of conf,
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        ...
        node distance=2.2cm,
        princ/.style={rectangle, draw=blue!75, fill=gray!10, text=black, text width=4.2cm, minimum height=3.2cm, rounded corners=4mm, align=center, font=\small\bfseries, line width=1.0pt},
        arrow/.style={-{Stealth[length=3mm]}, very thick, blue!80},
        feedback/.style={-{Stealth[length=3mm]}, very thick, red!80, dashed},
        labeltext/.style={font=\footnotesize\itshape, text=black!80}
        ]

        % Nodos principales
        \node[princ] (transp) at (0,0) {
        {\footnotesize TRANSPARENCIA}\\[1pt]
        {\scriptsize Convierte lo invisible en observable}
        };

        \node[princ] (insp) at (7.5,0) {
        {\footnotesize INSPECCIÓN}\\[1pt]
        {\scriptsize Monitorea evolución continua}
        };

        \node[princ] (adapt) at (3.75,-5.0) {
        {\footnotesize ADAPTACIÓN}\\[1pt]
        {\scriptsize Experimenta e interviene}
        };

        % Flechas del ciclo
        \draw[arrow] (transp.east) -- node[midway, above, labeltext, yshift=3pt] {hace visible} (insp.west);
        \draw[arrow] (insp.south) -- node[pos=0.55, right, labeltext, yshift=1pt, text width=3.2cm, align=center] {detecta desviaciones} (adapt.north);
        \draw[arrow] (adapt.north) -- node[pos=0.45, left, labeltext, yshift=1pt, text width=3.2cm, align=center] {genera cambios} (transp.south);

        % Feedback loop
        \draw[feedback] (adapt.east) .. controls +(2,-1.5) and +(0.5,-1.5) .. node[right, labeltext, text width=2cm, align=center] {nueva transparencia} (insp.south);

        % Etiqueta de cadencias
        \node[below=0.8cm of adapt, draw, fill=gray!10, rounded corners, text width=9.5cm, align=center, font=\footnotesize, text=black] {
            \textbf{Cadencias múltiples:} Diaria $\mid$ Sprint $\mid$ Release $\mid$ Continua
        };

        % Título superior
        \node[above=0.5cm of transp, xshift=3.75cm, font=\large\bfseries] {
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        node distance=0.3cm,
        colbox/.style={rectangle, draw=blue!75, fill=gray!10, text=black, text width=4.2cm, minimum height=5cm, rounded corners=4mm, align=center, font=\small\bfseries, line width=1.0pt},
        header/.style={\footnotesize\bfseries},
        item/.style={\scriptsize\rmfamily},
        connector/.style={-{Stealth[length=3mm]}, very thick, blue!80},
        labeltext/.style={font=\footnotesize\itshape, text=black!80}
        ]

        % Columna 1: ROLES
        \node[colbox] (roles) at (0,0) {
        {\footnotesize ROLES}\\[2pt]
        {\scriptsize
        Social Guide (integrador)\\[2pt]
        Scrum Master (facilitador)\\[2pt]
        Product Owner (priorizador)\\[2pt]
        Developers (ejecutores)}
        };

        % Columna 2: EVENTOS
        \node[colbox, right=1.8cm of roles] (eventos) {
        {\footnotesize EVENTOS}\\[2pt]
        {\scriptsize
        Daily Social\\[1pt]
        Sprint Planning Social\\[1pt]
        Sprint Review Social\\[1pt]
        Retrospective Social (núcleo)}
        };

        % Columna 3: ARTEFACTOS
        \node[colbox, right=1.8cm of eventos] (artef) {
        {\footnotesize ARTEFACTOS}\\[2pt]
        {\scriptsize
        Product Backlog Social\\[1pt]
        Sprint Backlog Social\\[1pt]
        Registro de Smells\\[1pt]
        Métricas Sociales}
        };

        % Conectores bidireccionales
        \draw[connector] (roles.east) -- node[midway, above, labeltext] {} (eventos.west);
        \draw[connector] (eventos.east) -- node[midway, above, labeltext] {} (artef.west);

        % Etiquetas inferiores
        \node[below=0.8cm of roles, font=\footnotesize\itshape, text=black] {Responsabilidad};
        \node[below=0.8cm of eventos, font=\footnotesize\itshape, text=black] {Cadencias};
        \node[below=0.8cm of artef, font=\footnotesize\itshape, text=black] {Transparencia};

        % Título superior
        \node[above=0.5cm of eventos, font=\large\bfseries] {
            Componentes Operacionales de SocialScrum
        };

        % Subtítulo
        \node[below=1.2cm of eventos, font=\footnotesize\itshape, text width=12cm, align=center] {
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[
        node distance=1.5cm,
        bigbox/.style={rectangle, draw=blue!75, fill=gray!10, text=black, text width=12cm, minimum height=3cm, rounded corners=4mm, align=left, font=\small\bfseries, line width=1.0pt},
        arrow/.style={-{Stealth[length=3mm]}, very thick, blue!80},
        labeltext/.style={font=\footnotesize\itshape, text=black}
        ]

        % Caja superior: Prevención
        \node[bigbox] (prev) at (0,0) {
        {\footnotesize Prevención de community smells}\\[2pt]
        {\scriptsize
        Comunicación: Radio Silence, Bottleneck\\
        Conocimiento: Black Cloud, Sharing Villainy\\
        Relacional: Lone Wolf, Toxic Relationships, Burnout\\
        Organizacional: Organizational Silo, Sluggish Organizations
        }
        };

        % Flecha central
        \draw[arrow] (prev.south) -- ++(0,-1.0);

        % Caja inferior: Resultados sistémicos
        \node[bigbox, below=2.0cm of prev] (result) {
        {\footnotesize Resultados sistémicos}\\[2pt]
        {\scriptsize
        $\downarrow$ Deuda Social \quad
        $\uparrow$ Confianza \quad
        $\uparrow$ Sostenibilidad\\[2pt]
        $\uparrow$ Cohesión \quad
        $\uparrow$ Autonomía \quad
        $\downarrow$ Burnout
        }
        };

        % Título superior
        \node[above=0.5cm of prev, font=\large\bfseries] {
            Outcomes de SocialScrum
        };

        % Subtítulo
        \node[above=0.2cm of prev, font=\footnotesize\itshape] {
            Qué se previene y qué mejora sistémicamente
        };

        % Nota al pie
        \node[below=0.5cm of result, font=\small\itshape, text width=12cm, align=center, text=black] {
...
\begin{figure}[p]
    ...
    \begin{tikzpicture}[
        node distance=1cm,
        % ================= Estilos =================
        layer/.style={rectangle, draw=black!60, fill=gray!20, text=black, text width=13cm, minimum height=1.2cm, rounded corners=2mm, align=center, font=\small\bfseries, line width=1pt},
        component/.style={rectangle, draw=black!60, fill=white, text=black, text width=3.8cm, minimum height=1.8cm, rounded corners=1.5mm, align=center, font=\scriptsize},
        outcomes/.style={rectangle, draw=black!60, fill=white, text=black, text width=12cm, minimum height=2cm, rounded corners=2mm, align=center, font=\scriptsize},
        arrow/.style={-{Stealth[length=3mm]}, very thick, blue!80},
        feedback/.style={-{Stealth[length=3mm]}, dashed, very thick, red!70}
        ]

        % ================= CAPA 2: PRINCIPIOS =================
        \node[layer] (capa2) at (0,0) {CAPA 2: PRINCIPIOS EMPÍRICOS};

        % Componentes de Capa 2 con fondo blanco
        \node[component, below=0.4cm of capa2, xshift=-4cm] (transp) {
        \textbf{Transparencia}\\[2pt]
        {\scriptsize Convierte invisible en observable}
        };
        \node[component, below=0.4cm of capa2, xshift=4cm] (insp) {
        \textbf{Inspección}\\[2pt]
        {\scriptsize Monitorea evolución continua}
        };
        \node[component, below=1.5cm of capa2] (adapt) {
        \textbf{Adaptación}\\[2pt]
        {\scriptsize Experimenta e interviene}
        };

        % ================= Flechas CAPA 2 =================
        % Transparencia -> Adaptación
        \draw[arrow] (transp.south) -- ++(0,-0.5) -- ++(-1.0,0) -- ([xshift=-0.2cm]adapt.north west);
        % Inspección -> Adaptación
        \draw[arrow] (insp.south) -- ++(0,-0.5) -- ++(1.0,0) -- ([xshift=0.2cm]adapt.north east);
        % Adaptación -> Transparencia
        \draw[arrow] (adapt.north) .. controls ++(0,1.5) and ++(0,-0.5) .. ([yshift=-0.1cm]transp.north);
        % Feedback loop Adaptación -> Inspección
        \draw[feedback] (adapt.east) .. controls ++(3,-0.5) and ++(0.5,-1.0) .. (insp.south);

        % ================= CAPA 3: COMPONENTES =================
        \node[layer, below=5cm of capa2] (capa3) {CAPA 3: COMPONENTES OPERACIONALES};

        % Componentes de Capa 3 con fondo blanco
        \node[component, below=0.3cm of capa3, xshift=-4.5cm] (roles) {
        \textbf{Roles}\\[1pt]
        {\scriptsize • Social Guide\\
        • Scrum Master\\
        • Product Owner\\
        • Developers}
        };
        \node[component, below=0.3cm of capa3] (eventos) {
        \textbf{Eventos}\\[1pt]
        {\scriptsize • Daily Social\\
        • Planning Social\\
        • Review Social\\
        • Retro Social}
        };
        \node[component, below=0.3cm of capa3, xshift=4.5cm] (artef) {
        \textbf{Artefactos}\\[1pt]
        {\scriptsize • Backlog Social\\
        • Registro Smells\\
        • Métricas}
        };

        % ================= Flechas Adaptación -> Componentes =================
        \draw[arrow] (adapt.south) -- ++(0,-0.5) -| ([yshift=0.2cm]roles.north west);
        \draw[arrow] (adapt.south) -- ++(0,-0.5) -| ([yshift=0.2cm]eventos.north);
        \draw[arrow] (adapt.south) -- ++(0,-0.5) -| ([yshift=0.2cm]artef.north east);

        % ================= CAPA 4: OUTCOMES =================
        \node[layer, below=5cm of capa3] (capa4) {CAPA 4: OUTCOMES};

        % Nodo Outcomes con fondo blanco
        \node[outcomes, below=0.3cm of capa4] (outcomes) {
            \textbf{Prevención:} Radio Silence, Black Cloud, Burnout, Silos\\[2pt]
            \textbf{Mejoras:} {\scriptsize $\downarrow$ Deuda Social $\mid$ $\uparrow$ Confianza $\mid$ $\uparrow$ Cohesión $\mid$ $\uparrow$ Sostenibilidad}
        };

        % ================= Flechas Componentes -> Outcomes (desde abajo hacia arriba) =================
        \draw[arrow] (roles.south) -- ++(0,-0.5) -- ++(0,0) -- (outcomes.north west);
        \draw[arrow] (eventos.south) -- ++(0,-0.5) -- ++(0,0) -- (outcomes.north);
        \draw[arrow] (artef.south) -- ++(0,-0.5) -- ++(0,0) -- (outcomes.north east);

        % Feedback loop Outcomes -> Transparencia
        \draw[feedback] (outcomes.east) .. controls +(4,0) and +(-1,-2) .. (transp.east);

        % ================= Etiquetas laterales =================
        \node[rotate=90, font=\scriptsize, anchor=south, text=black!80] at (-8, -1) {Cómo se aborda};
        \node[rotate=90, font=\scriptsize, anchor=south, text=black!80] at (-8, -7) {Qué implementa};
        \node[rotate=90, font=\scriptsize, anchor=south, text=black!80] at (-8, -13) {Qué mejora};

    \end{tikzpicture}
    \caption{\centering Arquitectura integrada del modelo SocialScrum (Capas 2-4) con flechas rodeando nodos y fondos grises}
...
\textbf{Sin un compromiso ejecutivo claro y tangible, SocialScrum no debe implementarse.}
