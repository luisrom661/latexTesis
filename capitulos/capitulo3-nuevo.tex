...
\paragraph{Límites y diferenciación claros}
...
\paragraph{Competencias requeridas}
...
El acceso del Social Guide a información sensible sobre dinámicas interpersonales implica una gran responsabilidad. Por ello, SocialScrum establece un conjunto de salvaguardas éticas y protecciones obligatorias que garantizan que dicha información nunca sea mal utilizada ni interpretada fuera de contexto.

\subsubsection{Principios éticos fundamentales}

\begin{enumerate}
    \item \textbf{Confidencialidad absoluta:} toda información compartida durante eventos sociales —como retrospectivas o sesiones de mediación 1:1— es estrictamente confidencial. Bajo ninguna circunstancia puede utilizarse para evaluaciones de desempeño, decisiones de despido u otras acciones laborales.

    \item \textbf{Rendición de cuentas clara:} el Social Guide reporta directamente al CTO o al Scrum Master, \textbf{nunca} a Recursos Humanos. Esta medida evita conflictos de interés y protege la naturaleza constructiva del rol, impidiendo que la información social se convierta en ``evidencia'' para decisiones administrativas o disciplinarias.

    \item \textbf{Transparencia hacia el equipo:} todos los reportes o hallazgos generados por el Social Guide son accesibles para el equipo. En SocialScrum no existen ``reportes secretos'' ni comunicaciones unilaterales sobre temas sociales.

    \item \textbf{Protección contra represalias:} cualquier miembro del equipo que denuncie una violación ética —por ejemplo, la divulgación indebida de información personal— está protegido y no puede ser objeto de sanciones o represalias.

\end{enumerate}

\subsubsection{Requisito de consentimiento informado}

Antes de implementar SocialScrum, es indispensable que el equipo otorgue su consentimiento informado. Este proceso implica:

\begin{itemize}
    \item \textbf{Sesión de explicación:} presentación clara del propósito de SocialScrum, los tipos de datos que se recopilan y los mecanismos establecidos para su protección.
    \item \textbf{Votación:} al menos el 70\% del equipo debe aprobar su adopción para garantizar que la implementación sea legítima y participativa.
    \item \textbf{Derecho a no participar:} cualquier persona puede optar por no participar en eventos sociales sin recibir represalias ni afectaciones en su rol dentro del equipo.
\end{itemize}

\subsubsection{Requisito de cambio Real, no teatro social}

Uno de los riesgos más importantes que enfrenta SocialScrum es caer en lo que se conoce como ``teatro social'': equipos que participan en retrospectivas o reuniones enfocadas en lo humano solo porque ``toca hacerlo'', pero sin generar transformaciones reales en las dinámicas del grupo. En esos casos, el proceso se vuelve una formalidad vacía que no aporta valor ni mejora la convivencia.

Para prevenirlo, SocialScrum establece un principio innegociable:

\begin{quote}
    \textit{``Toda acción identificada en una retrospectiva social debe implementarse antes del siguiente Sprint. Si el equipo no cuenta con los recursos necesarios, la situación debe escalarse al nivel de gestión o dirección.''}
\end{quote}

En la práctica, esto implica:

\begin{enumerate}
    \item Si el equipo detecta un conflicto, se debe crear un ítem de mediación dentro del Sprint.
    \item Si se identifica una falla de comunicación, se asignan recursos o tiempo para mejorar las herramientas y los procesos involucrados.
    \item Si se observa aislamiento entre los miembros, se promueven espacios de interacción genuina —como sesiones de \textit{\gls{pair-programming}} o talleres colaborativos—.
    \item Si la dirección no brinda apoyo o recursos, esta falta de acción debe documentarse y comunicarse formalmente. SocialScrum no puede sostenerse si las recomendaciones se ignoran de manera sistemática.
\end{enumerate}

Sin acciones concretas, el equipo se desmotiva y el proceso termina convirtiéndose en una fachada de ``bienestar'' que, lejos de ayudar, profundiza la frustración, por lo tanto, cualquier organización que adopte SocialScrum asume el compromiso de actuar sobre los hallazgos, no solo escucharlos. El cambio cultural y relacional debe ser tan tangible como los resultados técnicos.

Sin un consentimiento genuino, el proceso pierde su esencia colaborativa. La confianza —base fundamental de SocialScrum— no puede imponerse; debe construirse desde la transparencia y el respeto mutuo.

\emph{Referencia:} La operacionalización de los roles se desarrolla de manera detallada en el Capítulo 4, Sección 4.X.
...
\paragraph{Principio de No-Sobrecarga}

Los ajustes sociales se incorporan de la siguiente manera:
...
\emph{Referencia:} el Principio de No-Sobrecarga se desarrolla con mayor detalle en el Capítulo 4, Sección 4.X, donde se incluyen tablas de distribución de tiempo por evento y estrategias de optimización para reducir el overhead aún más.

Cada evento, por tanto, incorpora una dimensión social específica, alineada con los valores de Scrum y con los fundamentos teóricos del modelo. La Tabla \ref{tab:eventos_adaptados} muestra cómo cada ceremonia se transforma en un espacio para observar, evaluar y fortalecer la salud social del equipo; un punto de encuentro donde también se inspecciona lo humano, no solo lo técnico.
...
\emph{Nota: El principio de \textbf{No Sobrecarga} establece que estas adaptaciones deben integrarse en los eventos existentes de Scrum sin aumentar de forma significativa el tiempo total de las ceremonias. Este principio se desarrolla con mayor detalle en el Capítulo 4, Sección 4.X.}
...
La descripción completa del proceso —con facilitadores, tiempos sugeridos, preguntas guía y herramientas de apoyo— se desarrolla en el Capítulo 4, Sección 4.X, donde se presenta el manual práctico de implementación del modelo.
...
\paragraph{Diferenciación entre motivación intrínseca y extrínseca}
...
\paragraph{Conexión directa con el síndrome de Burnout}
...
\paragraph{Aplicabilidad demostrada en contextos de desarrollo de software}
...
\paragraph{Integración de dimensiones cognitivas y afectivas}
...
\paragraph{Diferenciación entre confianza y cooperación}
...
\paragraph{Validación empírica en equipos mediados por tecnología}
...
\paragraph{Operacionalización clara para la intervención}
...
No obstante, dado que SocialScrum está diseñado para implementarse tanto en equipos presenciales como en entornos remotos, el modelo de Mayer ofrece una base más amplia y adaptable. Su enfoque permite mantener coherencia conceptual incluso cuando las dinámicas de colaboración cambian según el contexto o la distancia. Esta idea se desarrollará con mayor detalle en la Sección 3.3.5.
...
Por ejemplo, cuando un desarrollador participa activamente en decisiones técnicas que lo afectan —como la elección de lenguajes, frameworks o procesos de trabajo—, suele sentirse más motivado, pues percibe que su voz tiene peso dentro del equipo. En cambio, cuando la dirección o el \textit{management} impone herramientas o metodologías sin consultar al grupo, la motivación tiende a disminuir: el desarrollador siente que ha perdido control sobre su propio entorno de trabajo.

SocialScrum busca precisamente evitar esa desconexión, promoviendo que el equipo participe en decisiones sobre herramientas de comunicación, normas de \textit{code review}, horarios o cualquier otro aspecto que influya en su dinámica diaria.
Esta participación no solo fortalece el compromiso, sino también el sentido de autonomía, un factor clave para la motivación genuina. De hecho, la investigación demuestra que depender en exceso de recompensas externas puede debilitar ese sentido de autonomía y, con ello, afectar la motivación intrínseca \cite{ryan_self-determination_2000}.
...
Si se ignoran estos aspectos, pueden presentarse escenarios poco saludables en los que el desarrollador solo recibe críticas vagas o comentarios sarcásticos. Esto debilita su sentido de competencia y lo lleva a pensamientos como: ``¿Qué hice mal?'', ``¿Podré mejorar?'' o incluso ``No sirvo para esto''.

En cambio, cuando después de una revisión de código el desarrollador recibe retroalimentación específica, constructiva y orientada al crecimiento, la situación cambia por completo. Su competencia se fortalece y surgen pensamientos mucho más sanos y motivadores: ``Esto lo hice bien'', ``Sé en qué puedo mejorar'' o ``Estoy creciendo profesionalmente``.

En esa línea, SocialScrum busca precisamente reforzar esa percepción de eficacia personal, promoviendo entornos donde la retroalimentación nutra y motive en lugar de desgastar. De esta forma, contribuye a prevenir la sensación de ineficacia profesional, uno de los síntomas más frecuentes del \textit{burnout} \cite{tulili2022burnout}.
...
Un ambiente colaborativo saludable permite, por ejemplo, que un desarrollador bajo presión pueda acudir a sus compañeros en busca de apoyo o que estos, al notar la situación, se ofrezcan espontáneamente a ayudar. Ese gesto genera un sentido de pertenencia y alimenta un pensamiento positivo: ``no estoy solo, mi equipo me respalda''.

Por el contrario, cuando cada desarrollador trabaja de forma aislada, sin interacción ni colaboración, se crea un entorno de silos que termina generando sentimientos de desconexión, soledad y falta de apoyo. En consecuencia, aparecen pensamientos negativos que afectan tanto la motivación como el rendimiento del equipo.

En SocialScrum, esta necesidad se aborda de manera directa, fomentando la cohesión grupal y reduciendo los \textit{community smells} asociados al aislamiento, como el conocido \textit{lone wolf} o ``lobo solitario'' \cite{caballero2022communitysmells}.
Cada intervención social busca fortalecer la seguridad psicológica dentro del equipo, promoviendo un espacio donde las personas puedan expresarse sin miedo, y donde la confianza y el respeto mutuo sean la base de las relaciones de trabajo.
...
Además del modelo de confianza de Mayer et al. \cite{mayer_integrative_1995}, durante el diseño de SocialScrum se tuvieron en cuenta otros enfoques teóricos relevantes, como la propuesta de Kim et al. \cite{kim2009individual}, quienes desarrollan una tipología de confianza especialmente útil para equipos distribuidos (100\% remotos o altamente dispersos). Este enfoque distingue tres formas de confianza:

\begin{enumerate}
    \item \textbf{Confianza Rápida (\textit{Swift Trust}):} se basa en roles y expectativas iniciales. Surge desde el primer día y permite que los equipos remotos comiencen a operar rápidamente, aunque es frágil y difícil de sostener sin interacción constante.
    \item \textbf{Confianza basada en el Conocimiento (\textit{Knowledge-Based Trust}):} se construye a partir de la experiencia compartida y del conocimiento que cada miembro tiene sobre las competencias y hábitos de los demás.
    \item \textbf{Confianza basada en la Identificación (\textit{Identification-Based Trust}):} es la forma más profunda, fundada en valores y objetivos compartidos. Su desarrollo requiere tiempo, pero genera relaciones más estables y resilientes.
\end{enumerate}

Esta tipología resulta especialmente útil en entornos de trabajo remoto, donde la confianza debe construirse de manera asincrónica y principalmente a través de medios digitales, sin la interacción presencial que suelen tener los equipos co-localizados.

En ese sentido, SocialScrum toma el modelo de Mayer como base general —por su solidez conceptual y aplicabilidad universal—, pero integra los matices de Kim et al. para adaptarse a contextos distribuidos.
En estos casos, la medición de la confianza (particularmente las dimensiones \textit{knowledge-based} e \textit{identification-based}, más allá de la \textit{swift trust}) se vuelve un indicador clave del funcionamiento social del equipo.
Por ello, la evaluación del valor ``Compromiso'' dentro del \textit{Health Score} incorpora preguntas específicas que permiten observar si el grupo ha avanzado desde una confianza inicial y funcional hacia una confianza más profunda, basada en el conocimiento y la identificación mutua.

En resumen, el modelo de Mayer aporta una estructura universal para entender la confianza en equipos, mientras que la tipología de Kim et al. enriquece esta perspectiva con matices prácticos para equipos remotos o distribuidos.
...
La forma de aplicar esta medición —incluyendo las preguntas específicas, las escalas de evaluación y el seguimiento de tendencias— se detalla en el Capítulo 4, Sección 4.X: \textit{``Métricas de Salud Social''}.
